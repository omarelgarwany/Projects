...
namespace App\Helpers\StoreRemote;
/** Change the namespace **/
...
class StoreRemote
{
    ...
    protected function setFile($uri) {
        $this->file = $uri;
    }
    ...
    protected function generateRandomName($length) {
        $this->full_filename = Str::random($length) . '.' . pathinfo(strtok($this->file, '?'), PATHINFO_EXTENSION);
    }

    //A method that wraps the UploadedFile move method and optimizes it for this class
    protected function mover() {
        copy($this->file, $this->location . '/' . $this->full_filename);
    }
     ...
     * @return string
     */
    public function move($uri, $location, $length = 16) {
        $this->setFile($uri);
